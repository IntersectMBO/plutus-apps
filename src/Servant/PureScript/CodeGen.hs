...
genModuleHeader moduleName imports =
  let importLines = map (strictText . importLineToText) . Map.elems $ imports
...
genLib =
  docIntercalate
    ...
    [ genClass
        ...
        [ Param "toURLPiece" $ mkPsType "a -> String" []
        ],
      docIntercalate
        (line <> "else ")
        [ genInstance
      ...
      genType "AjaxError" $
        genRecord
          [ Param "request" $ mkPsType "Request" [psJson],
            Param "description" $ mkPsType "ErrorDescription" []
          ],
      genData
        "ErrorDescription"
        [ mkPsType "UnexpectedHTTPStatus" [mkPsType "Response" [psJson]],
          mkPsType "DecodingError" [mkPsType "JsonDecodeError" []],
...
genDecl param =
  param ^. (pName . to psVar) <+> "::" <+> param ^. pType . to (strictText . typeInfoToText True)
...
genData name ctors =
  hang 2 $
    "data"
      <+> strictText name </> "="
      <+> docIntercalate (line <> "| ") (map (strictText . typeInfoToText True) ctors)

genClass :: PSType -> [Param PSType] -> Doc
genClass classType methods =
  hang 2 $
    "class"
      <+> strictText (typeInfoToText True classType)
      <+> "where"
  ...
  "instance"
    ...
    <+> ( if null constraints
            then mempty
              ...
              lparen
                <> docIntercalate ", " (map (strictText . typeInfoToText True) constraints)
                <> rparen
                <+> "=> "
        )
    <> strictText (typeInfoToText True instanceType)
    <+> "where"
    </> "  "
      ...
      <> ( if null constraints
             then mempty
               ...
               line
                 <> docIntercalate
                   (" =>" <> line)
                   (map (strictText . typeInfoToText True) constraints)
                 <+> "=>"
         )
      </> docIntercalate (" ->" <> line) (strictText . typeInfoToText True <$> (params <> [retType]))
  where
