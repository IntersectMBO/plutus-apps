...
genModule opts reqs =
  let allParams = concatMap reqToParams reqs
   ...
   in genModuleHeader (_apiModuleName opts) imports
...
genParamSettings rParams =
  let genEntry arg = arg ^. (pName . to psVar) <+> "::" <+> arg ^. pType . typeName . to strictText
      genEntries = docIntercalate (line <> ", ") . map genEntry
   in "newtype SPParams_ = SPParams_"
        <+/> align
          ( lbrace
              <+> genEntries rParams
              </> rbrace
...
genBuildQueryArgs args =
  "let queryArgs = catMaybes [" </> indent 2 (docIntercalate ("," <> softline) . map genBuildQueryArg $ args) </> "]"
