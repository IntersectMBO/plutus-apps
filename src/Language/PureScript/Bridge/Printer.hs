...
sumTypeToText settings st = sumTypeToTypeDecls settings st <> additionalCode
  where
    additionalCode =
...
sumTypeToTypeDecls settings (SumType t cs is) =
  T.unlines $ dataOrNewtype <> " " <> typeInfoToText True t <> " =" : "    " <>
...
instances settings st@(SumType t _ is) = map go is
  where
    ...
    recordUpdateString args =
      "{ " <> T.intercalate ", " (map (\(k, v) -> k <> " = " <> v) args) <> " }"
    go :: Instance -> Text
    go Encode =
      "instance encode" <> _typeName t <> " :: " <> extras <> "Encode " <>
      ...
      where
        ...
        instances params =
          genericInstance settings params <> ", " <> encodeInstance params
        bracketWrap x = "(" <> x <> ")"
    go Decode =
      "instance decode" <> _typeName t <> " :: " <> extras <> "Decode " <>
      typeInfoToText False t <>
      " where\n" <>
      ...
      where
        ...
        bracketWrap x = "(" <> x <> ")"
    go i =
      "derive instance " <> T.toLower c <> _typeName t <> " :: " <> extras i <>
      c <>
      " " <>
      typeInfoToText False t <>
      postfix i
      where
        ...
        genericConstraintsInner =
          T.intercalate ", " $ map (genericInstance settings) sumTypeParameters
        bracketWrap x = "(" <> x <> ")"
...
genericInstance settings params =
  if not (Switches.genericsGenRep settings)
    ...
    else "Generic " <> typeInfoToText False params <> " r" <>
         mergedTypeInfoToText params
...
recordOptics st@(SumType _ [_] _) =
  T.unlines $ recordEntryToLens st <$> dcRecords
  where
...
constructorToText _ (DataConstructor n (Left ts)) =
  n <> " " <> T.intercalate " " (map (typeInfoToText False) ts)
...
typeNameAndForall typeInfo = (typName, forAll)
  where
    ...
    forAllParams =
      typeInfo ^.. typeParameters . traversed . to (typeInfoToText False)
    forAll =
...
mkFnArgs rs =
  fromEntries (\recE -> recE ^. recLabel <> ": " <> recE ^. recLabel) rs
...
constructorToOptic ::
     Bool -> TypeInfo 'PureScript -> DataConstructor 'PureScript -> Text
  ...
  case (args, otherConstructors) of
    ...
    (Left cs, _) ->
      ...
      where mkF [] = n <> " = Just unit\n"
            mkF _ =
              "(" <> n <> " " <> T.unwords (map _recLabel types) <>
              ") = Just $ " <>
              mkFnArgs types <>
              "\n"
            getter
...
recordEntryToText e =
  _recLabel e <> " :: " <> typeInfoToText True (e ^. recValue)

typeInfoToText :: Bool -> PSType -> Text
typeInfoToText topLevel t =
  if needParens
    then "(" <> inner <> ")"
    else inner
  where
...
mergedTypeInfoToText t = _typeName t <> T.concat textParameters
  where
    params = _typeParameters t
