...
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint spends a script
-- output @txOutRef@ with redeemer @red@.
-- The script address is derived from the typed validator that is provided in
-- the 'Ledger.Tx.Constraints.OffChain.ScriptLookups' with
-- 'Ledger.Tx.Constraints.OffChain.typedValidatorLookups'.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that the
...
--   in @r@.
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint sets the
...
-- key @pk@.
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint adds @pk@ in the
-- transaction's public key witness set.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that @pk@
...
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint adds @dh@ and @d@
-- in the transaction's datum witness set.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that @dh@
...
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint adds @d@
-- in the transaction's datum witness set alongside it's hash
-- (which is computed automatically).
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that @d@
...
-- datum @d@.
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint creates a script
-- output with @d@ and @v@ and adds @d@ in the transaction's datum witness set.
-- The script address is derived from the typed validator that is provided in
-- the 'Ledger.Tx.Constraints.OffChain.ScriptLookups' with
-- 'Ledger.Tx.Constraints.OffChain.typedValidatorLookups'.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that @d@ is
...
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint creates a public key
...
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint mints a currency
...
-- 'Ledger.Tx.Constraints.OffChain.ScriptLookups' with
-- 'Ledger.Tx.Constraints.OffChain.typedValidatorLookups' or
-- 'Ledger.Tx.Constraints.OffChain.mintingPolicy'.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that the
...
-- be at least @v@.
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint adds the missing
-- input value with an additionnal public key output using the public key hash
-- provided in the 'Ledger.Tx.Constraints.OffChain.ScriptLookups' with
-- 'Ledger.Tx.Constraints.OffChain.ownPaymentPubKeyHash' and optionnaly
-- 'Ledger.Tx.Constraints.OffChain.ownStakePubKeyHash'.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that the
...
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint adds @utxo@ as an
-- input to the transaction. Information about this @utxo@ must be provided in
-- the 'Ledger.Tx.Constraints.OffChain.ScriptLookups' with
-- 'Ledger.Tx.Constraints.OffChain.unspentOutputs'.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that the
...
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint adds @utxo@ and
-- @red@ as an input to the transaction. Information about this @utxo@ must be
-- provided in the 'Ledger.Tx.Constraints.OffChain.ScriptLookups' with
-- 'Ledger.Tx.Constraints.OffChain.unspentOutputs'. The validator must be either provided by
-- 'Ledger.Tx.Constraints.OffChain.unspentOutputs' or through
-- 'Ledger.Tx.Constraints.OffChain.otherScript'. The datum must be either provided by
-- 'Ledger.Tx.Constraints.OffChain.unspentOutputs' or through
-- 'Ledger.Tx.Constraints.OffChain.otherData'.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that the
-- transaction spends this @utxo@.
...
--
-- If used in 'Ledger.Tx.Constraints.OffChain', this constraint checks that there's exactly one output that matches the requirements,
-- and then adds this as an input to the transaction with the given redeemer.
--
-- The outputs that will be considered need to be privided in the 'Ledger.Tx.Constraints.OffChain.ScriptLookups' with
-- 'Ledger.Tx.Constraints.OffChain.unspentOutputs'.
--
-- If used in 'Ledger.Tx.Constraints.OnChain', this constraint verifies that there's at least one input
