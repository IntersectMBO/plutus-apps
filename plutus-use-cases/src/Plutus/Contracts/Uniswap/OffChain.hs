...
import Ledger (OffchainTxOut, datumInDatumFromQuery, offchainTxOutScriptDatum, offchainTxOutValue, pubKeyHashAddress)
...
remove us RemoveParams{..} = do
    ...
    let usInst       = uniswapInstance us
        ...
        inVal        = view offchainTxOutValue o
...
add us AddParams{..} = do
    ...
    let outVal = view offchainTxOutValue o
...
pools us = do
  ...
  where
    ...
    go []       = return []
    go (o : os) = do
        let v = view offchainTxOutValue o
        if isUnity v c
            then do
                d <- getUniswapDatum o
                case d of
...
funds = do
    ...
    return $ mconcat [view offchainTxOutValue o | o <- os]
...
findUniswapInstance us c f = do
    ...
    utxos <- utxosAt addr
    go  [x | x@(_, o) <- Map.toList utxos, isUnity (view offchainTxOutValue o) c]
  where
