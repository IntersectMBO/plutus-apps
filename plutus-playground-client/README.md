...
The PureScript build depends on the presence of a `./generated` folder with bridge code generated by the backend. The folder generated the first time you run `plutus-playground-server`, and can be re-generated on demand by passing a `-g` flag like so: `plutus-playground-server -g`.
