...
`main` tracks the latest Cardano node version.
...
* `next-node` should regularly updates its node commit. Ideally we want `next-node` to depend on the tip of node master, but we are constrained by cardano-wallet, which is a PAB dependency and depends on the node.
