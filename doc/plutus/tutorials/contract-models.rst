...
field is not a slot number, it is a :hsobj:`PlutusLedgerApi.V1.Time.POSIXTime`. So while we shall
generate the deadline as a slot number (for convenience in the model),
we must convert it to a :hsobj:`PlutusLedgerApi.V1.Time.POSIXTime` before we can pass it to the
contract under test.
...
:hsobj:`Cardano.Node.Emulator.Internal.Node.TimeSlot.SlotConfig`,
a type defined in :hsmod:`Cardano.Node.Emulator.Internal.Node.TimeSlot`. In principle
...
.. note::
   ...
   its own :hsobj:`PlutusLedgerApi.V1.Time.POSIXTime` type with a resolution of milliseconds.
