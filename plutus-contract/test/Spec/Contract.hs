...
tests =
    let run :: String -> TracePredicate -> EmulatorTrace () -> _
        ...
        check :: String -> Contract eff () Schema ContractError () -> _ -> _
    ...
    testGroup "contracts"
        ...
        , let
            oneTwo :: Promise _ () Schema ContractError Int = endpoint @"1" pure .> endpoint @"2" pure .> endpoint @"4" pure
            oneThree :: Promise _ () Schema ContractError Int = endpoint @"1" pure .> endpoint @"3" pure .> endpoint @"4" pure
        ...
        , let theContract :: Contract _ () Schema ContractError () = void $ awaitPromise $ endpoint @"1" @Int pure .> endpoint @"2" @Int pure
          in run "call endpoint (1)"
                (endpointAvailable @"1" theContract tag)
                (void $ activateContract w1 theContract tag)

        -- , let theContract :: Contract eff () Schema ContractError () = void $ awaitPromise $ endpoint @"1" @Int pure .> endpoint @"2" @Int pure
        ...
        -- , let theContract :: Contract eff () Schema ContractError [ActiveEndpoint] = awaitPromise $ endpoint @"5" @[ActiveEndpoint] pure
        ...
        -- , let theContract :: Contract eff () Schema ContractError () = void $ submitTx mempty >> watchAddressUntilSlot someAddress 20
        --   in run "submit tx"
        ...
        --       theContract :: Contract eff () Schema ContractError () = submitTx smallTx >>= awaitTxConfirmed . getCardanoTxId >> submitTx smallTx >>= awaitTxConfirmed . getCardanoTxId
        ...
        --       theContract :: Contract eff () Schema ContractError () = void . awaitPromise $ selectEither l r
        --   in run "select either"
        --         (assertDone theContract tag (const True) "left branch should finish")
        --         (activateContract w1 theContract tag >>= (\hdl -> callEndpoint @"1" hdl 1 >> callEndpoint @"2" hdl 2))

        -- , let theContract :: Contract eff () Schema ContractError () = void $ loopM (\_ -> fmap Left . awaitPromise $ endpoint @"1" @Int pure) 0
        --   in run "loopM"
        --         (endpointAvailable @"1" theContract tag)
        --         (void $ activateContract w1 theContract tag >>= \hdl -> callEndpoint @"1" hdl 1)

        -- , let theContract :: Contract eff () Schema ContractError () =
...
checkpointContract :: Contract eff () Schema ContractError ()
...
loopCheckpointContract :: Contract eff () Schema ContractError Int
...
errorContract :: Contract eff () Schema ContractError Int
errorContract = do
    catchError
