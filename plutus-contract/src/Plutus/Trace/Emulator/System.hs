...
import Plutus.ChainIndex (ChainIndexControlEffect, ChainSyncBlock (EmulatorBlock), ChainSyncState (ChainSynced),
                          Tip (Tip, TipAtGenesis), appendBlocks, blockId, fromOnChainTx, getTip)
...
appendNewTipBlock lastTip block newSlot = do
  ...
  appendBlocks ChainSynced [(EmulatorBlock newTip (fmap (\tx -> (fromOnChainTx tx, def)) block))]
