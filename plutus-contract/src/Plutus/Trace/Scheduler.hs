...
mkSysCall prio sc = yield @(EmSystemCall effs systemEvent a) @(Maybe systemEvent) (WithPriority prio sc) id
...
sleep :: forall effs systemEvent effs2 a.
    ...
    => Priority
    -> Eff effs2 (Maybe systemEvent)
sleep prio = mkSysCall @effs @systemEvent @effs2 @a prio (Left WaitForMessage)
...
exit a = mkSysCall @effs @systemEvent @effs2 Normal (Right (Exit a))
...
loop s = do
    case dequeue s of
        AThread EmThread{_continuation, _threadId, _tag} event schedulerState prio -> do
            ...
            case result of
                Done _ -> loop $ schedulerState & removeActiveThread _threadId
                Continue WithPriority{_priority, _thread=sysCall} k -> do
                    ...
                    case newState of
                        Left a          -> logDebug (mkLog Stopped) >> pure (Just a)
