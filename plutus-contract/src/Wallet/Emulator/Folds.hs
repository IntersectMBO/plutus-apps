...
instanceState ::
    ...
    -> ContractInstanceTag
    -> EmulatorEventFoldM effs (Maybe (ContractInstanceState i o w s e a))
instanceState con tag =
    let flt :: EmulatorEvent -> Maybe (Response JSON.Value)
        ...
        decode :: forall effs'. Member (Error EmulatorFoldErr) effs' => EmulatorEvent -> Eff effs' (Maybe (Response o))
        decode e = do
            case flt e of
                Nothing -> pure Nothing
                Just response -> case traverse (JSON.fromJSON @o) response of
...
instanceRequests ::
    ...
    => Contract i o w s e a
    -> ContractInstanceTag
    -> EmulatorEventFoldM effs [Request i]
...
instanceTransactions con = fmap g . instanceState @PABReq @PABResp @w @s @e @a @effs con where
    g :: Maybe (ContractInstanceState PABReq PABResp w s e a) -> [UnbalancedTx]
...
instanceResponses ::
    ...
    => Contract i o w s e a
    -> ContractInstanceTag
    -> EmulatorEventFoldM effs [Response o]
