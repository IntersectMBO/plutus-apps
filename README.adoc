...
** The version of transitive dependencies (`plutus`, `cardano-ledger`, `ouroboros-network`) should be pinned to the ones from `cardano-node` (or better, `cardano-wallet`)
...
* It should use `cardano-wallet` release which is compatible with the current `cardano-node` release
...
* It may use `cardano-wallet` release which is incompatible with the current `cardano-node` release
