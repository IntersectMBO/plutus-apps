...
genInitialTransaction GeneratorModel{..} =
    let o = fmap (fromRight (error "did someone change gmInitialBalance?")) $ (\(ppk, v) -> pubKeyTxOut v ppk Nothing) <$> Map.toList gmInitialBalance -- FIXME
...
genValidTransactionSpending' g ins totalVal = do
    ...
    if fee' < totalValAda
        then do
            ...
            let txOutputs = map (fromRight (error "did someone change gmPubKeys?")) $ (\(v, ppk) -> pubKeyTxOut v ppk Nothing) <$> zip outVals (Set.toList $ gmPubKeys g) -- FIXME
                tx = EmulatorTx $ mempty
                        { txInputs = ins
                        , txOutputs = txOutputs
