...
fromPlutusIndex (P.UtxoIndex m) = first Right $
  UTxO . Map.fromList <$> traverse (\(k, v) -> do
    txOutRef <- fromPlutusTxOutRef k
    pure (txOutRef, fromPlutusTxOut v)
  )
    (Map.toList m)

fromPlutusTxOutRef :: V1.TxOutRef -> Either P.ToCardanoError (TxIn StandardCrypto)
fromPlutusTxOutRef (V1.TxOutRef txId i) = TxIn <$> fromPlutusTxId txId <*> pure (mkTxIxPartial i)
...
fromPlutusTxOut (P.TxOut txOut) = C.Api.toShelleyTxOut ShelleyBasedEraBabbage $ C.Api.toCtxUTxOTxOut txOut
