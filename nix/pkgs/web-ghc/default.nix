{ haskell, makeWrapper, runCommand, extraPackagesFun ? ps: [ ], gcc, writeShellScriptBin, bubblewrap, lib, util-linux }:
let
  ...
  runtimeGhcWrapped = writeShellScriptBin "runghc" ''
    export PATH=${lib.makeBinPath [ bubblewraps runtimeGhc util-linux gcc ]}
